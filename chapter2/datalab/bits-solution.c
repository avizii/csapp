...
int bitXor(int x, int y) {
    // solution key point: refer to practice problem 2.13
    // how to implement | operator by ~ &
    /*int a = x & ~y;
    int b = y & ~x;
    int c = ~a & ~b;
    return ~c;*/

    // distributive property & de morgan's law
    return ~(~x & ~y) & ~(x & y);
}
 ...
 */
int negate(int x) {
    // see chapter 2.3.3 Web Aside : Bit-level representation of two’s-complement negation
    // 1.complement the bits and then increment the result
    // 2.let k be the position of the rightmost 1, complement each bit to the left of bit position k
    return ~x + 1;
}
//3
/* 
